...
class VisitIPLocation
{
	...
	function getlocation($ip, $separator = 0) {
		...
		if (inp('纯真网络,CZ88.NET',$location['country'])>0){		// CZ88.NET表示没有有效信息
			$location['country'] = "";
		}
		if ($location['area'] == " CZ88.NET") {
			$location['area'] = "";
		}
		if(substr($location['area'],0,1)=='/') $location['area']=substr($location['area'],1);
		return $location;
	}
	 ...
	 * @return IpLocation
	 */
	function VisitIPLocation($filename = 'qqwry.dat')
	{
		$filename=appPath('data/qqwry.dat');
		$this->fp = 0;
