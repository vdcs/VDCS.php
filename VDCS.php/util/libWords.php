...
class libWords{
	...
	public function getPathDic(){return dirname(__FILE__) . '/';}
	...
	public function LoadDict($maindic='')
	{
		...
		//正常读取文件
		$dicAddon = $this->getPathDic() . $this->addonDicFile;
		if ($maindic == '' || !is_file($maindic)) {
		...
		//加载主词典（只打开）
		$this->mainDicHand = fopen($this->getPathDic() . $dicWords, 'r');
	...
	public function InportDict($targetfile)
	{
		if(!ini_set('memory_limit', '128M')) exit('设置内存错误');
		$zip = new utilZip();
		//echo $targetfile;
		$unpackagefile = array_keys($zip->Extract($targetfile,'/data/'));
		//exit();
		$this->MakeDict('/data/'.$unpackagefile[0]);
		unlink('/data/'.$unpackagefile[0]);
		return true;
	}
}
?>