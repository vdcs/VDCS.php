...
class utilFTP
{
	...
	//链接
	public function fconnect($ftphost,$ftpport,$ftpusername,$ftppassword,$ftppath,$ftpssl=0,$pasv=0,$tranmode=0,$timeout=0,$checkftp=0)
	{
	...
	//链接模式
	public function fPasv($pasv)
	{
		return @ftp_pasv($this->ftpconnectid,$pasv);
	}
	
	//改变路径
	public function fChdir($path)
	{
	...
	//设置目录权限
	public function fChmoddir($mode,$filename)
	{
		$mode=intval($mode);
	...
	//上传文件
	public function fTranFile($hfile,$lfile,$startpos=0,$del=0)
	{
	...
	//上传单文件(含建目录)
	public function fTranPathFile($basepath,$path,$hfile,$lfile,$del=0)
	{
		//建目录
		$this->mkdirs($basepath,$path);
		//上传文件
		$this->fTranFile($hfile,$lfile,0,$del);
	}
	
	//上传多文件
	public function fMoreTranFile($hfile,$lfile,$del=0)
	{
		$count=count($hfile);
		for($i=0;$i<$count;$i++){
	...
	//上传多文件(含建目录)
	public function fMoreTranPathFile($basepath,$path,$hfile,$lfile,$del=0)
	{
		//建目录
		$this->mkdirs($basepath,$path);
		//上传文件
	...
	//下载文件
	public function fGetFile($lfile,$hfile,$resumepos=0)
	{
		$hfile=$this->wipespecial($hfile);
		$lfile=$this->wipespecial($lfile);
	...
	//删除文件
	public function fDelFile($hfile)
	{
		$hfile=$this->wipespecial($hfile);
	...
	//删除多文件
	public function fMoreDelFile($hfile)
	{
		$count=count($hfile);
		for($i=0;$i<$count;$i++)
		{
	...
	//重命名文件
	public function fRename($oldfile,$newfile)
	{
		...
		while($file=@readdir($d)){
			if($file!= "."&&$file!=".."){
				if(is_dir($src_dir."/".$file)){
					$this->copydir($src_dir."/".$file,$dst_dir."/".$file);
				}
				else {
	...
	//删除目录
	public function rmdir($path,$flag=true)
	{
		$path=$this->wipespecial($path);
		if($flag){
