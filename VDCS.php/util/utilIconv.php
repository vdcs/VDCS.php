<?php
define('CODETABLEDIR', VDCS_CONFIG_PATH.'dat/');
class utilIconv
{
...
à - 2 bytes
	*/
	...
	public static function gbk_to_utf8($gbstr) {
		...
		while($gbstr) {
			if(ord(substr($gbstr, 0, 1)) > 0x80) {
				...
				$utf8 = '';
				@$utf8 = self::unicode_to_utf8(hexdec($CODETABLE[hexdec(bin2hex($thisW)) - 0x8080]));
				if($utf8 != '') {
	...
	public static function gbk_to_pinyin($str,$ishead=0,$isclose=1) {
		global $pinyins;
		$restr = '';
		$str = trim($str);
		$slen = strlen($str);
