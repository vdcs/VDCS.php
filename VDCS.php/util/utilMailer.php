<?
if (version_compare(PHP_VERSION, '5.0.0', '<') ) exit("Sorry, this version of utilMailer will only run on PHP version 5 or greater!\n");
...
class utilMailer {
	...
	*/
	...
	/**
	 * Path to utilMailer plugins.	Useful if the SMTP class
	...
	/**
	 * Sets the utilMailer Version number
	 * @var string
	 */
	...
	protected function AddAnAddress($kind, $address, $name = '') {
		if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
			...
			if ($this->exceptions) {
				throw new utilMailerException('Invalid recipient array: ' . $kind);
			}
			if ($this->SMTPDebug) {
		...
		if (!$this->ValidateAddress($address)) {
			...
			if ($this->exceptions) {
				throw new utilMailerException($this->Lang('invalid_address').': '.$address);
			}
			if ($this->SMTPDebug) {
	...
	public function PreSend() {
		try {
			...
			if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
				throw new utilMailerException($this->Lang('provide_address'), self::STOP_CRITICAL);
			}
			...
			if (empty($this->Body)) {
				throw new utilMailerException($this->Lang('empty_message'), self::STOP_CRITICAL);
			}
	...
	public function PostSend() {
		try {
			...
			switch($this->Mailer) {
				case 'sendmail':	return $this->SendmailSend($this->MIMEHeader, $this->MIMEBody);break;
				case 'smtp':		return $this->SmtpSend($this->MIMEHeader, $this->MIMEBody);break;
				//case 'mail':
				default:		return $this->MailSend($this->MIMEHeader, $this->MIMEBody);break;
			}
		} catch (utilMailerException $e) {
			$this->SetError($e->getMessage());
			if ($this->exceptions) {
		...
		if ($this->SingleTo === true) {
			foreach ($this->SingleToArray as $val) {
				if(!@$mail = popen($sendmail, 'w')) {
					throw new utilMailerException($this->Lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
				}
				fputs($mail, "To: " . $val . "\n");
				fputs($mail, $header);
				fputs($mail, $body);
	...
	protected function MailSend($header, $body) {
		...
		if(!$rt) {
			throw new utilMailerException($this->Lang('instantiate'), self::STOP_CRITICAL);
		}
		return true;
	}
	...
	protected function SmtpSend($header, $body) {
		//include_once($this->PluginDir . 'utilMailerSMTP.php');
		$bad_rcpt = array();

		if(!$this->SmtpConnect()) {
			throw new utilMailerException($this->Lang('smtp_connect_failed'), self::STOP_CRITICAL);
		}
		$smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
		if(!$this->smtp->Mail($smtp_from)) {
			$this->SetError($this->Lang('from_failed') . $smtp_from . " : " . implode(",",$this->smtp->getError())) ;
			throw new utilMailerException($this->ErrorInfo, self::STOP_CRITICAL);
		}
		...
		if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
			...
			throw new utilMailerException($this->Lang('recipients_failed') . $badaddresses);
		}
		if(!$this->smtp->Data($header . $body)) {
			throw new utilMailerException($this->Lang('data_not_accepted'), self::STOP_CRITICAL);
		}
				...
				if ($this->smtp->Connect(($ssl ? 'ssl://':'').$host, $port, $this->Timeout)) {
					...
					if ($tls) {
						if (!$this->smtp->StartTLS()) {
							throw new utilMailerException($this->Lang('connect_host'));
						}
					...
					if ($this->SMTPAuth) {
						if (!$this->smtp->Authenticate($this->Username, $this->Password, $this->AuthType,
							...
							throw new utilMailerException($this->Lang('authenticate'));
						}
					}
				}
				$index++;
	...
	*/
	function SetLanguage($langcode = 'en', $lang_path = 'language/') {
		...
		if ($langcode != 'en') { //There is no English translation file
			$l = @include $lang_path.'utilMailer.lang-'.$langcode.'.php';
		}
		$this->language = $utilMailer_LANG;
	...
	*/
	...
	public function CreateHeader() {
		...
		if ($this->XMailer == '') {
				$result .= $this->HeaderLine('X-Mailer', 'utilMailer '.$this->Version.' (http://code.google.com/a/apache-extras.org/p/utilMailer/)');
		} else {
		...
		if ($this->IsError()) {
			...
			try {
				...
				if (@openssl_pkcs7_sign($file, $signed, "file://".$this->sign_cert_file, array("file://".$this->sign_key_file, $this->sign_key_pass), NULL)) {
					...
					@unlink($file);
					@unlink($signed);
					throw new utilMailerException($this->Lang("signing").openssl_error_string());
				}
			} catch (utilMailerException $e) {
				$body = '';
				if ($this->exceptions) {
	...
	public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
		try {
			if ( !@is_file($path) ) {
				throw new utilMailerException($this->Lang('file_access') . $path, self::STOP_CONTINUE);
			}
	...
	*/
	...
	protected function EncodeFile($path, $encoding = 'base64') {
		try {
			if (!is_readable($path)) {
				throw new utilMailerException($this->Lang('file_open') . $path, self::STOP_CONTINUE);
			}
	...
	*/
	public function set($name, $value = '') {
		try {
			if (isset($this->$name) ) {
				...
				throw new utilMailerException($this->Lang('variable_set') . $name, self::STOP_CRITICAL);
			}
		} catch (Exception $e) {
			$this->SetError($e->getMessage());
			if ($e->getCode() == self::STOP_CRITICAL) {
	...
	$to_header						= "";
		...
		return "X-utilMailer-DKIM: code.google.com/a/apache-extras.org/p/utilMailer/\r\n".$dkimhdrs.$signed."\r\n";
	}
...
/**
 * Exception handler for utilMailer
 * @package utilMailer
 */
class utilMailerException extends Exception {
	/**
