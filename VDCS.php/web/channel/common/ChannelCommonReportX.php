<?
class ChannelCommonReportX extends ChannelCommonBaseX
{
	...
	public function doLoad()
	{
		...
		$this->TableFields='r_id,channel,rootid,dataid,uurc,uuid,r_realname,r_email,r_title,r_url,r_topic,r_remark,sp_code,sp_ip,sp_agent,r_status,r_tim';
	...
	public function doParsePost()
	{
		...
		$ctl->treeData->addItem('r_url',$url);
		if($this->ua->isLogin()){
			$ctl->treeData->addItem('r_realname',query('realname'));
			$ctl->treeData->addItem('r_email',query('email'));	
		}
		...
		if($ctl->e->isCheck()){$ctl->doRaiseError();}
			...
			$ctl->treeData->addItem('r_trans_name','');
