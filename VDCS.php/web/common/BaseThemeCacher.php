...
class BaseThemeCacher
{
	...
	*/
	public function getIncludeContent($file)
	{
		...
		if(left($file,1)==DIR_SEPARATOR){
			...
			$filepath=$this->getIncludePathReal('theme',$file);
		}
		else{
			$filepath=$this->getIncludePathReal('channel',$file);
		}
		if(isFile($filepath)){
	...
	public function getIncludePathReal($pathc,$file)
	{
		static $paths=[];
		if(!$paths[$pathc]){
			$patha=null;
			switch($pathc){
				case 'theme':
					$patha=[
						$this->_var['theme.path'],
						$this->_var['themedef.path'],
						$this->_var['themeapp.path'],
						$this->_var['themebase.path'],
					];
					break;
				case 'channel':
					$patha=[
						$this->_var['channel.path'],
						$this->_var['channelc.path'],
						$this->_var['channeldef.path'],
						$this->_var['channelvc.path'],
						$this->_var['channelv.path'],
						$this->_var['channelapp.path'],
						$this->_var['channelbase.path'],
					];
					break;
			}
			if(!$patha) return null;
			$paths[$pathc]=$patha;
			if($this->_var['subdirs']){
				$paths[$pathc]=[];
				foreach($patha as $path){
					array_push($paths[$pathc],$path,$path.$this->_var['subdirs']);
				}
			}
		}
		return self::toIncludePathReal($paths[$pathc],$file,$this->_var['ext']);
	}
	public static function toIncludePathReal($patha,$file,$ext)
	{
		$filepath='';
		foreach($patha as $path){
			$filepath=$path.$file;
			if($ext) $filepath=appExt($filepath,$ext);
			//debugx($filepath);
	...
	*/
	...
	public function getXCMLItem($file,$k,$terms,$items='')
	{
		...
		$re=$tableData->getTermsValue($terms,$k);
		$re=VDCSDTML::toParseItems($re,$items);
		return $re;
	}
