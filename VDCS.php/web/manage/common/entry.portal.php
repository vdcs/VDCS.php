<?
global $_cfg;
//debugx('MANAGE_CHANNEL_NOW='.MANAGE_CHANNEL_NOW.';PAGE_P='.PAGE_P.';PAGE_M='.PAGE_M.';PAGE_MI='.PAGE_MI.';PAGE_X='.PAGE_X);
$_cfg['entry']['filepath']=ManageCommon::entryPath(MANAGE_CHANNEL_NOW,PAGE_P,PAGE_M,PAGE_MI,PAGE_X);
$_cfg['entry']['file']=$_cfg['entry']['filepath'];
//debugx('--'.$_cfg['entry']['filepath']);
$_cfg['entry']['_class']=false;
if(!isFile($_cfg['entry']['filepath'])){
	$className='Portal'.ucfirst(PAGE_CHN).ucfirst(PAGE_P).ucfirst(PAGE_M).ucfirst(PAGE_MI).ucfirst(PAGE_X);
	//debugx($className);
