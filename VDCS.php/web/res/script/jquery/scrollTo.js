...
$scrollTo.both=function(val){return typeof val == 'object' ? val : { top:val, left:val }};
...
$.fn.scrollTo = function( target, duration, settings ){
	...
	settings.offset = $scrollTo.both( settings.offset );
	settings.over = $scrollTo.both( settings.over );
	return this._scrollable().each(function(){
		...
		switch( typeof targ ){
			...
			case 'string':
				if( /^([+-]=)?\d+(\.\d+)?(px|%)?$/.test(targ) ){
					targ = $scrollTo.both( targ );
					break;
				}
				targ = $(targ,this);
			case 'object':
				if( targ.is || targ.style ) toff = (targ = $(targ)).offset();				// Get the real position of the target 
		}
