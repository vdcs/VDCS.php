...
SWFUpload.prototype.initSettings = function () {
	...
	if (!this.settings.preserve_relative_urls) {
		...
		//dbg.t("upload_url: "+this.settings.upload_url);
		//dbg.t("button_image_url: "+this.settings.button_image_url);
	}
...
SWFUpload.prototype.loadFlash = function () {
	...
	if (targetElement == undefined) throw "Could not find the placeholder element: " + this.settings.button_placeholder_id;

	// Append the container and load the flash
	tempParent = document.createElement("div");
	tempParent.innerHTML = this.getFlashHTML();	// Using innerHTML is non-standard but the only sensible way to dynamically add Flash in IE (and maybe other browsers)
	//targetElement.parentNode.replaceChild(tempParent.firstChild, targetElement);
	//targetElement.replaceChild(tempParent.firstChild,targetElement);
	targetElement.innerHTML=tempParent.innerHTML;
	...
	try {
		...
		if (movieElement && typeof(movieElement.CallFunction) === "unknown") { // We only want to do this in IE
			...
			for (var i in movieElement) {
				try {
					if (typeof(movieElement[i]) === "function") movieElement[i] = null;
				} catch (ex1) {}
			}
